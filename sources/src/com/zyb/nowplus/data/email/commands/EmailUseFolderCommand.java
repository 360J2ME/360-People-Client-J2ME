 ...
 * @author Jens Vesti
 */
public class EmailUseFolderCommand extends EmailCommand {

	private final String folder;
	
	public EmailUseFolderCommand(final String folder) {
		this.folder = folder;
	}

	public byte[][] getSerialisedCommand() {
		String command = this.getCurrentId()+" SELECT "+folder;
		return new byte[][] {command.getBytes()};
	}
	
	public EmailResponse createResponse(String response) {
		return new EmailUseFolderResponse(response);
	}
	
	//#mdebug error
	public String toString() {
		return "EmailUseFolderCommand[folder=" + folder
		+ "]";
	}
	//#enddebug	
}
