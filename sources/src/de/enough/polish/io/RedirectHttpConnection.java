...
public class RedirectHttpConnection implements HttpConnection
{
	...
	{
		//#debug debug
		System.out.println("\n\nEntering ensureConnectionCreated");
		...
		while (true)
		{
					...
					|| resultCode == HttpConnection.HTTP_TEMP_REDIRECT)
			{
				...
				int lastIdx = TextUtil.lastIndexOf(url, "://") + "://".length();
			    int indexOf = url.indexOf('/', lastIdx);
			    String server = url.substring(0, indexOf);
			    
			    url = tmpHttpConnection.getHeaderField("Location");

				//#debug debug
				System.out.println("URL 2: "+server);
				//#debug debug
				...
				//#debug debug
				System.out.println("REDIRECT #: "+redirects);
				continue;
			}
		...
		//#debug debug
		System.out.println("FINAL URL: "+url+" AFTER #: "+redirects+" REDIRECTS!");
	...
	public String getEncoding()
	{
		try {
			...
			//#debug error
de.enough.polish.util.Debug.debug("error", "de.enough.polish.io.RedirectHttpConnection", 449, "Unable to establish connection", e);
	...
	public long getLength()
	{
		try {
			...
			//#debug error
de.enough.polish.util.Debug.debug("error", "de.enough.polish.io.RedirectHttpConnection", 466, "Unable to establish connection", e);
	...
	public String getType()
	{
		try {
			...
			//#debug error
de.enough.polish.util.Debug.debug("error", "de.enough.polish.io.RedirectHttpConnection", 483, "Unable to establish connection", e);
